...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.2")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.2")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.2: 단타 5분봉은 유지하면서 거래일별 선택/복기, 전일·다음일 이동, 전체보기 날짜 구분선, 과거 날짜 분석 시 미래 데이터 차단을 추가.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
