...
def fetch_manifest(manifest_url: str, timeout: int = 12) -> UpdateInfo:
    ...
    req = urllib.request.Request(manifest_url, headers={"User-Agent": "PUMA-STOCK-UPDATER/0.9"})
...
def download_package(info: UpdateInfo, progress_cb=None) -> Path:
    ...
    req = urllib.request.Request(info.url, headers={"User-Agent": "PUMA-STOCK-UPDATER/0.9"})
