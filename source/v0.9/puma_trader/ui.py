...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v0.9")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v0.9")
    ...
    def _focus_tab(self):
        ...
        self.focus_splitter.setSizes([235, 855, 390])
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v0.9: 메인 오른쪽 영역 스크롤/폭 개선, 역매공파 별도 중복 탭 제거 후 메인 통합 유지, 단타 분석 전용 화면 추가.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
    ...
    def focus_refresh(self):
        ...
        try:
            ...
            # 단타는 별도 '단타 분석' 화면에서 판정한다. 메인에서는 데이터만 미리 적재한다.
            try:
                minute = self.broker.get_minute_candles(code, 5)
