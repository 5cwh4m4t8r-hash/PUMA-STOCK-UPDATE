...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.5")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.5")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.5: 사용자 영웅문 화살표 3종을 수식 그대로 반영. 분홍/파랑/빨강 신호를 일봉·5분봉 차트에 색상별 매수 화살표로 표시.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
