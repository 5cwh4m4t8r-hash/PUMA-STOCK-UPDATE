...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.4")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.4")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.4: 직접 주문 명칭 변경, 분석 카드에 간단 이유 표시, 기존 임시 수박바 제거, 일봉/5분봉에 일목균형표 선행스팬1·2 파란 구름대 반영.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
