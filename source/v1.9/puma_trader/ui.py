...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.9")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.9")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.9: YTN형 급등 기준봉-거래량 감소 눌림을 일반 패턴으로 추가. 영웅문 화면에 맞춰 EMA5/20/60/112/224/448 색·굵기 재정리, 224 검정선/검정화살표는 흰 외곽선으로 대비 강화. 역매공파 설정값을 전부 노출하고 PUMA 표준값 버튼 추가.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
