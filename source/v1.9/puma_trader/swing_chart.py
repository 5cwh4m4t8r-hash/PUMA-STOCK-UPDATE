...
class SwingChart(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for c in cs:
            vals += [c['high'], c['low']]
        line_keys = [k for k in self.series.keys() if k not in ('candles','acc_flags','acc_meta','box','watermelon','watermelon_stage','watermelon_score','watermelon_reason','cloud_a','cloud_b','signal_pink','signal_blue','signal_red','signal_black','signal_sar','signal_bb40_22') and isinstance(self.series.get(k), list)]
