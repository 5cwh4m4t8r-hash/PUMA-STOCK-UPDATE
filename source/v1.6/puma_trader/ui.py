...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.6")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.6")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.6: 공개 차트/강의 특징을 바탕으로 PUMA 수박근사 1~3단계 추가, 과거 유사신호 TP/SL 적중률과 PUMA 진입판단 표시, 모든 콤보박스 마우스휠 값변경 차단.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
