...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v2.4")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v2.4")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v2.4: 차트 상단의 EMA/기준선/파란점선/일목/수박 지표명 범례를 숨겨 차트를 정리. 분홍·파랑·빨강·검정 4종 화살표를 큰 화살표+흰 외곽선+개별 lane으로 그려 겹침과 검정배경 묻힘을 개선. 검정 화살표 발생식은 사용자 수식 전까지 임의 생성하지 않음.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
