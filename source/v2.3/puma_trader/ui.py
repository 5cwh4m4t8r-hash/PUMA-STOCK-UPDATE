...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v2.3")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v2.3")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v2.3: 공개 단테 강의의 박스돌파→저거래량 눌림 원칙을 재반영. 박스권 돌파는 전봉 또는 20봉평균 대비 거래량강도 300%로 확인하고, 눌림은 돌파선 지지와 뚜렷한 거래량 감소를 확인해 최초 1회 표시. 지나친 필터를 완화해 유효 돌파/눌림 누락을 줄이고 공통경로 품질점수를 확률판단에 사용.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
