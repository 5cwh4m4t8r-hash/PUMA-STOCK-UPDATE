...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v2.2")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v2.2")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v2.2: 박스 기간 고정을 제거하고 실제 수평 지지·저항 왕복구간만 공구리로 판정. 돌파/눌림/재돌파 라벨은 강화조건을 모두 통과한 최초 확정봉만 표시. 확률은 현재와 동일한 확정 단계만 따로 계산하고 표본수·95%구간·신뢰도를 함께 표시.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
