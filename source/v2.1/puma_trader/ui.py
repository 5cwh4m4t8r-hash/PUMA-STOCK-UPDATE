...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v2.1")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v2.1")
    ...
    def _focus_tab(self):
        ...
        box = QGroupBox("공구리(박스권) · 60~112봉 동적 탐지")
        bf = QFormLayout(box)
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v2.1: 모든 기법의 최상위 로직을 수급→전고점/박스상단 돌파→거래량감소 눌림→거래량동반 재돌파로 통합. 공구리는 60~112봉 동적 박스, 돌파/재돌파 거래량 300% 이상을 기본값으로 적용.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
