...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.1")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.1")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.1: 조건검색 단타/스윙/중장기 자동분류, 메인에 단타 분석 재통합, 차트 하단 PUMA 수박형 밴드, 숫자 입력을 휠 없는 직접입력+드롭다운 방식으로 변경.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
