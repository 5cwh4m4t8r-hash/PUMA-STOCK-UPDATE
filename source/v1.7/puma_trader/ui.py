...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.7")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.7")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.7: 전략3종 카드 잘림 전면 수정, 직접주문 가격 드롭다운을 현재가 주변 호가로 변경, 분봉 기준선26 가시성 강화, 일봉 EMA5 복구, 거래량 확대, 수박형 마커 숫자 제거·가시성 강화, 전역 휠 차단 안전망 추가.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
    ...
    def focus_refresh(self):
        ...
        try:
            ...
            # 단타 분석은 5분봉 고정. 화면에서는 거래일별로 분리/복기할 수 있다.
            danta_analysis = None
    ...
    def _apply_focus_analyses(self, danta_analysis, swing_analysis, bowl_analysis, label: str):
        if danta_analysis:
            ...
            dprob_short = str(dprob).split(" (", 1)[0]
        ...
        if swing_analysis:
            ...
            sprob_short = str(sprob).split(" (", 1)[0]
        ...
        if bowl_analysis:
            ...
            bprob_short = str(bprob).split(" (", 1)[0]
