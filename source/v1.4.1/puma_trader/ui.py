...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.4.1")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.4.1")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.4.1: 거래량 막대 색상을 직전 봉 대비 거래량 증감 기준으로 수정. 증가=빨강, 감소=파랑, 동일=회색.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
