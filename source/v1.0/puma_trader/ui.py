...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.0")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.0")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.0: 전 화면 레이아웃 재구성, 메인 우측 기능탭, 단타 설정 탭화, 해상도 자동배치/분할비율 저장, 역매공파 설정 영구저장.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
