...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.3")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.3")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.3: 일봉에는 날짜 경계선을 표시하지 않도록 정리하고, 조건검색 종목명 열 폭/글꼴/조회 우선순위를 개선해 종목명이 확실히 보이도록 수정.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
