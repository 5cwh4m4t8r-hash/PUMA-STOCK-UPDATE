...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v2.0")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v2.0")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v2.0: 전략별 핵심 진입 신호를 명시하고 '현재 신호 아님' 대신 미충족 조건을 직접 표시. YTN형 기준봉 눌림은 독립 유효신호로 인정. 수박은 고확률 복합조건을 통과한 확정구간만 20봉당 1회 표시.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
