...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PUMA STOCK PRO v1.8")
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        title = QLabel("🐆  PUMA STOCK PRO  v1.8")
    ...
    def check_update(self):
        ...
        if not url:
            ...
            self.update_notes.setText("v1.8: 매집봉 오탐 감소를 위해 단독 신호를 후보로 강등하고 20봉 내 2회 이상 반복 시에만 매집확정. 하단 전략3종은 말줄임/툴팁을 제거하고 카드 내부 세로 스크롤로 전체 분석을 확인하도록 변경.")
            self.update_apply_btn.setEnabled(False)
            return
        try:
