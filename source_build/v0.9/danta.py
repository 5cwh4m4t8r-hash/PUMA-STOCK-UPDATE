...
def analyze_danta(
    ...
    *,
    ...
    공개적으로 설명된 '5분봉 / 기준선 / 아침 거래량' 개념을
    기계적으로 판정할 수 있게 PUMA 방식으로 조합한 해석판이다.
    특정 유료/비공개 검색식이나 독점 공식을 복제하지 않는다.
    """
    ...
    score = 0
    score += 10 if in_time else 0
    score += 25 if baseline_lift else (10 if baseline_alive else 0)
    score += 15 if ema_stack else 0
    score += 15 if volume_ratio_5m >= 1.8 else (8 if volume_ratio_5m >= 1.2 else 0)
    score += 15 if day_volume_ratio >= 1.0 else (8 if day_volume_ratio >= 0.6 else 0)
    score += 10 if breakout else (7 if pullback_hold else 0)
    score += 10 if steep else 0
    score = min(100, score)

    candidate = bool(in_time and baseline_alive and score >= 70 and (breakout or pullback_hold))
    if candidate and baseline_lift and (volume_ratio_5m >= 1.8 or day_volume_ratio >= 1.0):
        stage = "타점 후보"
    ...
    details = {
        "분석 시간": f"{hm} / 기준 {scan_start}~{scan_end} · {'확인' if in_time else '시간외'}",
