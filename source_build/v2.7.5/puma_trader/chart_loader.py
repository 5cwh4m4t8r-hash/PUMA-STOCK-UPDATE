...
class FocusDataThread(QThread):
    ...
    def run(self):
        ...
        payload = dict(code=self.code, info={}, minute=[], daily=[], daily_pages=self.daily_pages,
                       request_id=self.request_id, errors=[], complete=False, timings={})
        done = {}
        ...
        def receive(kind):
            ...
            try:
                ...
                first_key = kind + '_first_ms'
                total_key = kind + '_total_ms'
                timings.setdefault(first_key, elapsed)
                timings[total_key] = round(float(timings.get(total_key, 0.0)) + elapsed, 2)

        def snapshot(complete, include_revision=True):
            revision = data_revision(payload['minute'], payload['daily']) if include_revision else ''
            return {**payload, 'errors': list(payload['errors']), 'timings': dict(payload['timings']),
                    'complete': complete, 'loaded_at': time.time(), 'revision': revision,
                    'fetch_elapsed_ms': round((time.perf_counter() - started) * 1000, 2)}
        ...
        try:
            ...
            if not self.isInterruptionRequested() and hasattr(broker, 'get_stock_info'):
                ...
                try:
                    ...
                    payload['timings']['quote_ms'] = round((time.perf_counter() - quote_started) * 1000, 2)
            if self.isInterruptionRequested():
                return
