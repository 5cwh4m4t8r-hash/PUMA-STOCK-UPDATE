...
def load_update_config() -> dict:
    ...
    try:
        ...
        if not url or url == "https://raw.githubusercontent.com/5cwh4m4t8r-hash/PUMA-STOCK-UPDATE/main/manifest-v271.json":
            # Heal v2.7 blank config and migrate the temporary v2.7.1 cache-bypass URL.
...
def fetch_manifest(manifest_url: str, timeout: int = 12) -> UpdateInfo:
    ...
    req = urllib.request.Request(manifest_url, headers={"User-Agent": "PUMA-STOCK-UPDATER/2.8.6"})
...
def download_package(info: UpdateInfo, progress_cb=None) -> Path:
    ...
    req = urllib.request.Request(info.url, headers={"User-Agent": "PUMA-STOCK-UPDATER/2.8.6"})
