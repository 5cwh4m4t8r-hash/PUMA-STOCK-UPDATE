...
def build_arrow_signals(candles: List[dict]) -> dict:
    ...
    for i in range(n):
        ...
        # PUMA 검정 화살표 근사.
        # 공개된 원본 검정 화살표 수식은 확인되지 않았다.
        # 기존 112(파랑) / 224(빨강) 장기선 계열과 대칭되게
        # SAR 위에서 EMA448을 상향돌파한 경우만 보수적으로 표시한다.
        black[i] = bool(sar_ok and x448[i])
    return {
        "signal_pink": pink,
        "signal_blue": blue,
        "signal_red": red,
...
def latest_signal_reason(series: dict) -> str:
    ...
    if i < len(series.get("signal_black", [])) and series["signal_black"][i]:
        names.append("검정(PUMA근사): SAR 위 + EMA448 상향돌파")
