...
ui = replace_once(ui, 'self.setWindowTitle("PUMA STOCK PRO v0.7")',
                  'self.setWindowTitle("PUMA STOCK PRO v0.8")',
                  "window version")
ui = replace_once(ui, """        self.resize(1560, 950)""",
                  """        self.setMinimumSize(1024, 680)
        self.resize(1280, 800)""",
                  "window geometry")
ui = replace_once(ui, 'title = QLabel("🐆  PUMA STOCK PRO  v0.7")',
                  'title = QLabel("🐆  PUMA STOCK PRO  v0.8")',
                  "header version")
ui = replace_once(ui,
                  'QTabBar::tab { background:#13253b; padding:9px 18px; margin-right:2px; }',
...
# ----- explicit tab refs for shared selected stock -----
...
# ----- strategy tab selected-stock banner -----
ui = replace_once(ui,
...
# Name lookup should propagate to every tab.
ui = replace_once(ui,
...
# ----- cross-tab synchronization helpers -----
...
# Selecting stock must be immediate.
ui = replace_once(ui,
...
# Latest quote/name push into all linked tabs.
ui = replace_once(ui,
"""            self.focus_title.setText(f"{name}  {code}")
...
# Swing detail auto-load can be silent.
ui = replace_once(ui,
                  '    def swing_load_kiwoom(self):',
...
ui = ui.replace(
    "v0.7: 통합 트레이딩 화면 + 키움 보안저장/자동연결 + 빠른연결이 추가되었습니다.",
    "v0.8: 해상도 최적화 + 통합/전략상세/역매공파/고급주문 선택종목 실시간 연동 + 종목 클릭 버그 수정."
)

# Startup geometry is always maximized to the current monitor working area.
